# -*- coding: UTF-8 -*-
# filename: low_level_keyboard_listener date: 2018/11/8 15:37  
# author: FD
...
def listen():
    ...
    def low_level_handler(nCode, wParam, lParama):
        """
        Processes a low level Windows keyboard event.
        """
        event = KeyboardEvent(event_types[wParam], lParama[0], lParama[1],
                              lParama[2] == 32, lParama[3])
        for handler in handlers:
            handler(event)
        # return 1 not received by other windows
        # Be a good neighbor and call the next hook.
        return windll.user32.CallNextHookEx(hook_id, nCode, wParam, lParama)
