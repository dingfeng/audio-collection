...
def get_feature(dir, filename):
    ...
    T=1.0/fs
    mfcc_feature_0 = mfcc(data[:,0], samplerate=fs, winlen=len(data)*T, winstep=len(data)*T,nfft=len(data),highfreq=200)
    mfcc_feature_1 = mfcc(data[:,1], samplerate=fs, winlen=len(data)*T, winstep=len(data)*T,nfft=len(data),highfreq=200)
...
if __name__ == '__main__':
    root_dir = 'label_data_dingfeng_recent_test'
    ...
    for index, item in enumerate(dir_filenames):
        ...
        mfcc_feature=mfcc_feature[0:12]
        tags.append(tag)
        filenames.append(filename)
        mfcc_feature = (mfcc_feature - np.min(mfcc_feature)) / (np.max(mfcc_feature) - np.min(mfcc_feature))
        mfcc_features.append(mfcc_feature)


    feature_dir = 'features'
    feature_file = os.path.join(feature_dir, 'dingfeng_12_200hz_recent.npz')
